#!/usr/bin/env python
"""Run seed-matched raw text-only baselines and residual Part 1 models."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", type=Path, default=Path("artifacts/residual_multiseed"))
...
def main() -> None:
    ...
    for seed in args.seeds:
        raw_output = args.output_root / f"none_raw_seed{seed}"
        run_train(
        ...
        for conditioning_type in args.conditioning_types:
            residual_output = args.output_root / f"{conditioning_type}_residual_seed{seed}"
            run_train(
                train_script=train_script,
                rda_path=args.rda_path,
